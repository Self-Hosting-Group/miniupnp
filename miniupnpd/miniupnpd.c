...
static void update_disable_port_forwarding(void)
{
	...
	if (r < 0) {
		...
		if (!reserved || GETFLAG(REPORTPRIVATEIPV4MASK)) {
			...
			syslog(LOG_WARNING, "- Info, as port maps not reachable via internet without an unrestricted endpoint-independent (1:1) IPv4 CGNAT, but possibly from within");
...
init(int argc, char * * argv, struct runtime_vars * v)
{
	...
	syslog(LOG_INFO, "Options default_min/max_port set to %d/%d apply to IPv6 and to IPv4 only, with no matching or default reject ACL entries",
...
#ifdef ENABLE_LEASEFILE
	if (lease_file && lease_file[0] != '\0') {
		syslog(LOG_INFO, "Reloading port maps from lease file %s", lease_file);
	...
	if(GETFLAG(ENABLEUPNPMASK))
	{
		...
		if(!GETFLAG(IPV6DISABLEDMASK)) {
			if(find_ipv6_addr(lan_addrs.lh_first ? lan_addrs.lh_first->ifname : NULL,
				...
				syslog(LOG_INFO, "Address given to UPnP IGDv2 IPv6 clients: %s",
