...
AddPortMapping(struct upnphttp * h, const char * action, const char * ns)
{
	...
	if (inet_pton(AF_INET, int_ip, &result_ip) <= 0)
	{
		...
		{
			syslog(LOG_INFO, "Failed to parse or resolve to (%s) IPv4 address via UPnP IGD", int_ip);
			ClearNameValueList(&data);
			SoapError(h, 402, "Invalid Args");
			return;
		}
	...
	syslog(LOG_INFO, "Add IPv4 port map %hu:%s:%hu/%s remote IP %s lifetime %u via UPnP IGD",
...
AddAnyPortMapping(struct upnphttp * h, const char * action, const char * ns)
{
	...
	if (inet_pton(AF_INET, int_ip, &result_ip) <= 0)
	{
		...
		{
			syslog(LOG_INFO, "Failed to parse or resolve to (%s) IPv4 address via UPnP IGDv2 AddAny", int_ip);
			ClearNameValueList(&data);
			SoapError(h, 402, "Invalid Args");
			return;
		}
	...
	case 0:	/* success */
		...
		syslog(LOG_INFO, "Add IPv4 port map %hu:%s:%hu/%s remote IP %s lifetime %u via UPnP IGDv2 AddAny",
...
/*! \brief Check the security policy rights for pinhole.
 * Pinhole InternalClient address must match the action sender.
 * if int_ip is a hostname, converts it to litteral ipv6 into int_ip_resolved
 * \param[in] h upnphttp object
 * \param[in] int_ip InternalClient argument or real internal client IP
 * \param[in] int_port InternalPort argument
 * \param[out] int_ip_resolved a INET6_ADDRSTRLEN long char buffer
 * \return 1 if it passes, call SoapError() and returns 0 or -1 if it fails
 */
static int
PinholeVerification(struct upnphttp * h, const char * int_ip, unsigned short int_port, char * int_ip_resolved)
{
	char clientaddr_str[INET6_ADDRSTRLEN];
	struct in6_addr result_ip;

	if (int_ip_resolved != NULL)
	{
		int_ip_resolved[0] = '\0';
		/* if ip not valid assume hostname and convert */
		if (inet_pton(AF_INET6, int_ip, &result_ip) <= 0)
		{
			...
			if (r != 0)
			{
				syslog(LOG_INFO, "Failed to parse or resolve to (%s) IPv6 address via UPnP IGDv2 IPv6",
				       int_ip);
				SoapError(h, 402, "Invalid Args");
				return -1;
			}
			for(p = ai; p; p = p->ai_next)
			{
				if(p->ai_family == AF_INET6)
				{
					...
					else
					{
						...
						syslog(LOG_INFO, "%s: additional IPv6 for \"%s\" : %s",
						       "PinholeVerification", int_ip, tmp);
					}
				}
			}
			freeaddrinfo(ai);
		...
		{
			/* it was already OK */
			strncpy(int_ip_resolved, int_ip, INET6_ADDRSTRLEN);
			int_ip_resolved[INET6_ADDRSTRLEN-1] = '\0';
		}
	}
	else
	{
	...
	if(memcmp(&h->clientaddr_v6, &result_ip, sizeof(struct in6_addr)) != 0)
	{
		syslog(LOG_INFO, "Reject IPv6 port map [%s]:?/? via UPnP IGDv2 IPv6 reason for third-party from %s",
		       int_ip_resolved == NULL ? int_ip : int_ip_resolved, clientaddr_str);
		SoapError(h, 606, "Action not authorized");
		return 0;
	}
#ifdef DEBUG
...
AddPinhole(struct upnphttp * h, const char * action, const char * ns)
{
	...
	char int_ip_resolved[INET6_ADDRSTRLEN];
	int uid = 0;
	...
	syslog(LOG_INFO, "Add IPv6 port map [%s]:%hu/%s remote IP %s remote port %hu lifetime %d via UPnP IGDv2 IPv6",
	       int_ip_resolved, iport, proto_itoa(proto), rem_host[0] != '\0' ? rem_host : "any", rport, ltime);
	...
	r = upnp_add_inboundpinhole(rem_host, rport, int_ip_resolved, iport, proto, "IGD2 pinhole", ltime, &uid);
...
UpdatePinhole(struct upnphttp * h, const char * action, const char * ns)
{
	...
	if (n >= 0)
	{
		if(PinholeVerification(h, iaddr, iport, NULL) <= 0)
			return;
	}
	else if(n == -2)
	{
...
GetOutboundPinholeTimeout(struct upnphttp * h, const char * action, const char * ns)
{
	...
	syslog(LOG_DEBUG, "%s: retrieving timeout for outbound pinhole from [%s]:%hu to [%s]:%hu protocol %s", action, int_ip, iport,rem_host, rport, protocol);
...
DeletePinhole(struct upnphttp * h, const char * action, const char * ns)
{
	...
	{
		if(PinholeVerification(h, iaddr, iport, NULL) <= 0) {
			syslog(LOG_INFO, "Non-existing IPv6 port map to delete via UPnP IGDv2 IPv2 UID %d", uid);
			return;
		}
	}
	else if(n == -2)
