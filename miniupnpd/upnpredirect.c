              ...
              unsigned int leaseduration)
{
	...
	if(!check_upnp_rule_against_permissions(upnppermlist, num_upnpperm,
		...
		syslog(LOG_INFO, "Reject IPv4 port map "
			"%hu:%s:%hu/%s remote_ip=%s lifetime=%u via=UPnP-IGD reason=ACL", eport, iaddr, iport, protocol, rhost[0] != '\0' ? rhost : "any", leaseduration);
		return -3;
	}
	...
	if(r == 0) {
		if(strcmp(iaddr, iaddr_old)==0 &&
		   ((rhost == NULL && rhost_old[0]=='\0') ||
			...
			syslog(LOG_INFO, "Renew IPv4 port map %hu:%s:%hu/%s remote_ip=%s via=UPnP-IGD",
				eport, iaddr_old, iport_old, protocol, rhost_old[0] != '\0' ? rhost_old : "any");
