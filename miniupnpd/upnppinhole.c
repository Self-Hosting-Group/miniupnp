                        ...
                        int * uid)
{
	...
	*uid = upnp_find_inboundpinhole(raddr, rport, iaddr, iport, proto, NULL, 0, NULL);
	if(*uid >= 0) {
		syslog(LOG_INFO, "Renew IPv6 port map [%s]:%hu/%s remote_ip=%s remote_port=%hu via=UPnP-IGDv2-IPv6 uid=%d", iaddr, iport, proto_itoa(proto), raddr[0] != '\0' ? raddr : "any", rport, *uid);
